...
Once we knew what the potential problem is we fixed it by extending our object invariant with a forall clause to look like this:
...
Now, we are making sure that all the elements in the \emph{seatReservations} array are of type Customer.

\subsection{Eliminating ``violation of modifies clause'' warnings}

The \emph{toString} method calls a few other methods in the class but none of them modifies any state. ESC/Java was complaining about \emph{indexToNumber} so we marked the method as pure. This means that the method doesn't modify any instance variables. After this simple change the Modifies warning stopped appearing. We are not entirely sure why this works though as the method is also marked as a helper which means it should be inlined. That would imply that it does not need to be marked as pure for this to work.
